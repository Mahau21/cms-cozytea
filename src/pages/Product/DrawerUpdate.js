...
export default function Product({ visible, onClose, item, count }) {
  ...
  async function onFinish(values) {
    try {
      setLoading(true)
      if (item) {
        await apiProduct.update(item.id, values)
  ...
  return (
    ...
    >
      <Form {...layout} name="update_product" ref={formRef} onFinish={onFinish} form={form}>
        <Form.Item
          label="Tên product"
          name="name"
