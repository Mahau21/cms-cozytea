...
export default function Navbar({ items, isNav }) {
  ...
  return (
    <div
      className={`bg-slate-900 z-10 sticky text-slate-400 h-screen border-r border-slate-700 top-0 shadow ease-in-out duration-300 ${wapperClass}`}
    >
      <Link to="/">
        <div
          className={`gap-3 border-b border-slate-700 px-6 h-14 flex text-xl font-bold items-center text-slate-50`}
        >
          <img src="/logo180.svg" alt="logo" className="h-10 w-10" />
          {isNav && "Onedu"}
        </div>
      </Link>
            ...
            {items.map((item) => {
              ...
              if (children && children.length > 0) {
                ...
                const activeClassName = location.pathname.includes(href)
                  ...
                  : "text-slate-400"
                return (
                  <li key={href} className="px-4 mb-2">
                    <NavLink to={href}>
